...
채점 구현과 항목별 실제 검증 상태는 [검증된 채점 체크리스트](docs/features/verified-grading-checklist.md)를 참고한다.
...
- 인증: 기존 팀 인증이 `app.api.dependencies.get_current_user`를 제공/override해야 한다. 별도 로그인은 구현하지 않았다.
- DB: 운영에서는 Alembic/PostgreSQL 구성을 사용한다.
- 자원 제한 수치와 코드 최대 크기, 배틀 점수, 일일 보상은 제품 정책 확정 전까지 TBD다.
